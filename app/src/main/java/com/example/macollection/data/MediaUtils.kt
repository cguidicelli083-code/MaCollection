...
object MediaUtils {
    ...
    /**
     * Télécharge une photo distante (jaquette IGDB/RAWG, photo Wikipédia, résultat de
     * recherche...) et la copie dans le stockage interne, EXACTEMENT comme une photo importée à
     * la main ([copyToInternal]). Sans ça, la fiche ne conserve qu'un lien internet : si l'URL
     * expire, change ou est inaccessible plus tard (pas de réseau, hébergeur qui la retire), la
     * photo disparaît de la fiche même si elle est restée intacte dans la base. Best-effort :
     * renvoie null en cas d'échec réseau (l'appelant garde alors l'URL distante en repli).
     */
    suspend fun downloadToInternal(context: Context, url: String): String? = withContext(Dispatchers.IO) {
        try {
            val request = Request.Builder()
                .url(url)
                .header("User-Agent", "Mozilla/5.0 (Android) MaCollection/2.0 (import de photo)")
                .build()
     ...
     * Sans effet sur les URLs distantes (catalogue, jaquettes en ligne).
     */
