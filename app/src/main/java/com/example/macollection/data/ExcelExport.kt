 ...
 * photo par ligne (la photo principale uniquement). Pas de vraie librairie .xlsx (lourde sur
 * Android) : on génère un document **MHTML** (page web "fichier unique", multipart/related) —
 * Excel l'ouvre nativement (avec un avertissement de compatibilité de format, sans consequence).
 *
 * Important : une image en data-URI directement dans le `<img src="data:...">` du HTML ne
 * s'affiche PAS de façon fiable dans le moteur de rendu d'Excel. Le format MHTML, lui, référence
 * chaque image via `cid:` et la transporte comme une pièce jointe MIME séparée (`Content-ID`) —
 * c'est la méthode native que Word/Excel utilisent eux-mêmes pour les pages web "fichier unique",
 * donc fiable. On garde aussi la compression en miniature (plus rapide à charger, fichier plus léger).
 */
object ExcelExport {

    private const val THUMBNAIL_MAX_DIMENSION = 200
    private const val THUMBNAIL_JPEG_QUALITY = 70
    private const val BOUNDARY = "----MACOLLECTION_EXPORT_BOUNDARY"
    ...
    suspend fun export(context: Context, items: List<CollectionItem>, destUri: Uri): Boolean =
        withContext(Dispatchers.IO) {
            try {
                ...
                htmlBody.append(
                    "<tr>" +
                        "<th>Photo</th><th>Statut</th><th>Type</th><th>Nom</th><th>Marque</th>" +
                        "<th>Région</th><th>État</th><th>Boîte</th><th>Notice</th><th>Année</th>" +
                        "<th>Genre</th><th>Plateforme</th><th>Code-barres</th><th>Prix</th><th>Description</th>" +
                        "</tr>"
                )
                for (item in items) {
                    var photoCell = ""
                    ...
                    htmlBody.append("<td>${escapeHtml(item.type.label)}</td>")
                    htmlBody.append("<td>${escapeHtml(item.name)}</td>")
                    htmlBody.append("<td>${escapeHtml(item.brand)}</td>")
                    htmlBody.append("<td>${escapeHtml(item.region.label)}</td>")
                    htmlBody.append("<td>${escapeHtml(item.condition.label)}</td>")
                    htmlBody.append("<td>${if (item.hasBox) "Oui" else "Non"}</td>")
                    htmlBody.append("<td>${if (item.hasManual) "Oui" else "Non"}</td>")
                    htmlBody.append("<td>${item.releaseYear ?: ""}</td>")
                    htmlBody.append("<td>${escapeHtml(item.genre.orEmpty())}</td>")
                    htmlBody.append("<td>${escapeHtml(item.platform.orEmpty())}</td>")
                ...
                mhtml.append("Content-Location: macollection.html\r\n\r\n")
